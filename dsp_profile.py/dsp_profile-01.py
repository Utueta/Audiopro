...
class StftProfile:
    ...
    All fields are intended to be logged for forensic traceability and to be treated
    as part of the DSP profile version contract.
    """
    ...
    # Normalization conventions (flags are explicit to prevent silent drift)
...
def compute_stft_profile_from_sr(
    ...
    *,
    ...
    Rules:
      - Use Hann (periodic) window (STFT/FFT convention).
      - Choose n_fft from fixed time-window duration: n_fft = next_pow2(round(sr * win_sec)).
      - Clamp n_fft to [n_fft_min, n_fft_max] for scalability.
      - hop_length = n_fft * (1 - overlap_ratio), rounded to nearest integer, min 1.
      - Use float64 processing and power spectrum, window-power normalization, and fixed eps.
    """
