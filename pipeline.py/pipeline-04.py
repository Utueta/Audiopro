...
class AudioAnalysisPipeline:
    ...
    def _blake2b_3point_hash(self, file_path: str) -> Tuple[str, Dict[str, float]]:
        ...
        # 128-bit identity (digest_size=16) is sufficient for dedupe & telemetry
    ...
    def _select_segments(self, duration_s: float, seed: int) -> List[Tuple[float, float]]:
        """Deterministic segmented loading strategy (stratified sampling).

        Segments:
          - First 30s (intro quality)
          - Middle 10s (representative body)
          - Last 10s (outro/fade artifacts)
          - Random 5s segment (deterministic via seed)
        """
        if duration_s <= 0:
            return [(0.0, 0.0)]
    ...
    def analyze_file(self, file_path: str, is_segmented: Optional[bool] = None) -> AnalysisResult:
        ...
        UI may override segmentation via is_segmented.
        If is_segmented is None, config decides.
        """
        ...
        if is_segmented is None:
            ...
            mode = "segmented" if bool(is_segmented) else "full"

        metadata["loading_mode"] = mode
        was_segmented = (mode == "segmented")
        ...
        for (_off, _dur, y, seg_sr) in segments:
            ...
            ms_ratio, _iacc = stereo_integrity_metrics(y, seg_sr, compute_iacc=False)
        ...
        if pyln is not None and segments and segments[0][3] > 0:
            try:
                ...
                # Heuristic: very specific loudness targets may indicate normalization
                metadata["loudness_normalization_suspect"] = bool(-15.5 <= lufs <= -13.5 or -24.5 <= lufs <= -22.5)
            except Exception:
                pass
