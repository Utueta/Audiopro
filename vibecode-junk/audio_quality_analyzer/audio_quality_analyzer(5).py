...
Installation des dépendances pour LINUX:
sudo apt update
...
IMPORTANT: Les fichiers marqués comme défectueux ne seront plus jamais analysés.
"""
...
class LearningDatabase:
    ...
    def __init__(self, db_file: str = "audio_learning_db.json"):
        ...
        self.data = self.load()
        self.defective_files_cache = self._build_defective_cache()
    ...
    def load(self) -> Dict:
        ...
        return {
            ...
            'defective_files': {}  # hash -> chemin complet
        }
    ...
    def _build_defective_cache(self) -> set:
        """Construit un cache des chemins de fichiers défectueux pour accès rapide"""
        cache = set()
        # Ajouter depuis les échantillons d'entraînement
        for sample in self.data.get('training_samples', []):
            if sample['user_label'] == 'defective':
                cache.add(sample['path'])
        # Ajouter depuis le dictionnaire de fichiers défectueux
        cache.update(self.data.get('defective_files', {}).values())
        return cache
    
    def is_defective(self, file_path: str) -> bool:
        """
        Vérifie si un fichier a déjà été marqué comme défectueux
        
        Args:
            file_path: Chemin du fichier à vérifier
            
        Returns:
            True si le fichier est marqué comme défectueux
        """
        return file_path in self.defective_files_cache
    ...
    def add_sample(self, file_data: Dict, user_label: str, user_comment: str = ""):
        ...
        # Si marqué comme défectueux, l'ajouter à la liste des fichiers à exclure
        if user_label == 'defective':
            file_hash = self.calculate_file_hash(file_data['path'])
            if file_hash:
                if 'defective_files' not in self.data:
                    self.data['defective_files'] = {}
                self.data['defective_files'][file_hash] = file_data['path']
                self.defective_files_cache.add(file_data['path'])
        
        self.save()
    
    def get_excluded_count(self) -> int:
        """Retourne le nombre de fichiers exclus (défectueux)"""
        return len(self.defective_files_cache)
...
class AudioAnalyzerGUI(QMainWindow):
    ...
    def setup_analysis_tab(self):
        ...
        excluded = self.analyzer.learning_db.get_excluded_count()
        learning_info = QLabel(
            f"📚 Base d'apprentissage: {stats['total_reviewed']} fichiers évalués "
            f"(Défectueux: {stats['defective']}, Bonne qualité: {stats['good']}) | "
            f"⛔ {excluded} fichiers exclus des futures analyses"
        )
    ...
    def scan_files(self):
        ...
        for ext in audio_extensions:
            all_files.extend(Path(folder).rglob(f'*{ext}'))
            all_files.extend(Path(folder).rglob(f'*{ext.upper()}'))
        
        all_files = list(set([str(f) for f in all_files]))
        
        # Filtrer les fichiers déjà marqués comme défectueux
        excluded_count = 0
        self.audio_files = []
        for file_path in all_files:
            if not self.analyzer.learning_db.is_defective(file_path):
        ...
        total_valid = len(self.audio_files)
        
        stats_msg = f"✅ {total_found} fichiers audio trouvés"
        if excluded_count > 0:
            stats_msg += f" | ⛔ {excluded_count} fichiers exclus (déjà marqués défectueux)"
        stats_msg += f" | 📊 {total_valid} fichiers à analyser"
        
        self.scan_stats.setText(stats_msg)
        self.log(f"✅ {total_found} fichiers trouvés, {total_valid} à analyser ({excluded_count} exclus)")
    ...
    def mark_file(self, label):
        ...
        log_msg = f"✅ {file_data['file']} marqué comme {label_text}"
        
        if label == 'defective':
            log_msg += " (sera exclu des prochaines analyses)"
